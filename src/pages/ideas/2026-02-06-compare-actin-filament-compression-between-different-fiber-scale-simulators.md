...
ideaDetails:
  ...
  introduction: >-
    ...
    at monomer- and fiber-scales using the [ReaDDy](https://readdy.github.io/)
    and [Cytosim](https://gitlab.com/f-nedelec/cytosim) simulation engines,
    respectively, under different compression velocities. To better understand
    how the differences we observed may be specific to the choice of simulators,
    we proposed running an additional comparison between two simulators at fiber
    scale.
  ...
  preliminaryFindings:
    summary: Fiber-scale simulations of single actin filament compression (using
      Cytosim) were not able to capture filament supertwist behavior observed
      with monomer-scale simulations (using ReaDDy).
    figures:
      - type: imageLink
